...
from .layers import (GlobalConditionBlock2,GlobalConditionBlock3,InputProj, CondProj, Downsample, Upsample, Fovea, PromptGenBlock)
from basicsr.utils.registry import ARCH_REGISTRY

@ARCH_REGISTRY.register()
class CUNet_prompt(nn.Module):

    def __init__(self, in_nc=3, out_nc=3, nf=32, act_type='relu'):
        super(CUNet_prompt, self).__init__()
        # base
        self.conv_first = nn.Conv2d(in_nc, nf, 3, 1, 1)
        
        self.SFT_layer1g = SFTLayer()
        ...
        cond_in_nc=3
        cond_nf=32
        self.Global_CondNet = GlobalConditionBlock3(in_nc, cond_nf)
        ...
        # prompt
        # self.prompt1 = PromptGenBlock(prompt_dim=nf,prompt_len=8,prompt_size = 16,lin_dim = nf)
        self.prompt1 = PromptGenBlock(prompt_dim=nf,prompt_len=8,prompt_size = 32,lin_dim = nf)
        self.prompt2 = PromptGenBlock(prompt_dim=nf,prompt_len=8,prompt_size = 16,lin_dim = nf)
        self.convfuse1 = nn.Conv2d(nf*2, nf, kernel_size=1)
        self.convfuse2 = nn.Conv2d(nf*2, nf, kernel_size=1)
    ...
    def forward(self, x, gcond):
        ...
        gcond = self.Global_CondNet(gcond)
        ...
        # prompt 2
        dec2_param = self.prompt2(out)
        out = torch.cat([out, dec2_param], 1)
        out = self.convfuse2(out)
        out = self.act(self.up_conv1(out)) + fea1
        out, _, _ = self.recon_trunk3((out, gcond, cond2))
        # prompt 1
        dec1_param = self.prompt1(out)
        out = torch.cat([out, dec1_param], 1)
        out = self.convfuse1(out)
